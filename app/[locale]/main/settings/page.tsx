...
const SettingPage = () => {
    ...
    if (error) {
        console.error(error);
        return <ErrorComponent item={dict.setting.errorItem} />;
    }

    const fileName = `${user?.id}/avatar.webp`;

    const DangerButton = ({ handleClick, content, isRed }: { isRed: boolean; content: string; handleClick: () => void; }) => {
        return <button type="button" onClick={handleClick} className={`text-sm py-2 rounded-md font-bold ${isRed ? 'bg-rose-200 text-rose-500' : 'bg-foreground text-muted-foreground icon hover:bg-extreme'}`}>{content}</button>;
    };
    ...
    const handleDeleteAccount = async () => {
        ...
        if (!ok || !session?.access_token) return;
    ...
    const processAvatar = (file: File, maxSizeKB: number = 50): Promise<Blob> => {
    ...
    const handleUpdateFormData = (e: ChangeEvent<HTMLInputElement, HTMLInputElement>) => {
        setFormData(prev => ({
            ...prev,
