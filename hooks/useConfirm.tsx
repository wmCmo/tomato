        ...
        <div onClick={() => onClose(false)} className="fixed inset-0 grid place-items-center bg-background/60 backdrop-blur-lg text-accent px-4">
            <div onClick={e => e.stopPropagation()} className="card py-8 text-center max-w-lg w-full space-y-6 px-4">
                ...
                <div className="space-x-6 font-semibold">
                    <button type="button" className={`min-w-28 bg-muted px-4 py-2 rounded-lg text-accent ${buttonAnimation}`} onClick={() => onClose(false)}>{dict.ui.cancel}</button>
                    <button type="button" className={`min-w-28 bg-rose-500 px-8 py-2 rounded-lg text-white ${buttonAnimation}`} onClick={() => onClose(true)}>{dict.ui.delete}</button>
                </div>
            </div>
        </div>
    ) : null;
