...
let lowTick: HTMLAudioElement | null = null;
function getAudio() {
    if (!tick && !lowTick && typeof window !== 'undefined') {
        tick = new Audio('/stopwatch.mp3');
        tick.preload = 'auto';
        lowTick = new Audio('/lap.mp3');
...
export default function Stopwatch() {
    const [laps, setLaps] = useState<{ id: Date; lap: number; total: number; }[]>([]);
    ...
    function handleLaps() {
        setLaps(prev => {
            ...
            return [{ id: new Date(), lap: sec - lastTotal, total: sec }, ...prev];
        });
    }
    ...
    return (
        <div className="select-none text-accent mt-8 sm:min-w-106 flex flex-col grow min-w-80 max-w-md justify-center transition-all duration-400 ease-in-out w-full py-8 px-6 bg-foreground shadow-md rounded-xl gap-8">
            ...
            <div className="select-none flex grow gap-4 justify-center">
                <button type="button" onMouseDown={() => getAudio().lowTick?.play()} onClick={counting ? handleLaps : handleReset} disabled={sec === 0} className={`bg-background px-4 py-2 rounded-full min-w-24 ${sec === 0 ? 'opacity-50' : 'active:scale-90 active:opacity-80'}`}>{counting ? dict.stopwatch.lap : dict.stopwatch.reset}</button>
                <button type="button" onMouseDown={() => getAudio().tick?.play()} onClick={() => setCounting(prev => !prev)} className={`bg-background px-4 py-2 rounded-full min-w-24 active:scale-90 active:opacity-80 ${counting ? 'bg-rose-400' : 'bg-blue-500'} text-white`}>{counting ? dict.stopwatch.pause : sec === 0 ? dict.stopwatch.start : dict.stopwatch.resume}</button>
            </div>
            {
                laps.length > 0 &&
                <div className="space-y-4 max-h-48 overflow-y-scroll scrollbar-muted">
                    ...
                    {laps.map((lap, index) => {
                        ...
                        return (
                            <div key={lap.id.toISOString()} className="grid grid-cols-3 gap-4">
