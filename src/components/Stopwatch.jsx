...
export default function Stopwatch() {
    ...
    function handleLaps() {
        setLaps(prev => {
            ...
            return [{ lap: sec - lastTotal, total: sec }, ...prev];
        });
    }
    ...
    return (
        <div className="select-none text-accent mt-8 sm:min-w-[424px] flex flex-col flex-grow min-w-80 max-w-md justify-center transition-all duration-400 ease-in-out w-full py-8 px-6 bg-foreground shadow-md rounded-xlpy-8 rounded-xl gap-8">
            ...
            <div className="select-none flex flex-grow gap-4 justify-center">
                <button onClick={counting ? handleLaps : handleReset} disabled={sec === 0} className={`bg-background px-4 py-2 rounded-full min-w-24 ${sec === 0 ? 'opacity-50' : 'active:scale-90 active:opacity-80'}`}>{counting ? dict.stopwatch.lap : dict.stopwatch.reset}</button>
                <button onClick={() => setCounting(prev => !prev)} className={`bg-background px-4 py-2 rounded-full min-w-24 active:scale-90 active:opacity-80 ${counting ? 'bg-rose-400' : 'bg-blue-500'} text-white`}>{counting ? dict.stopwatch.pause : sec === 0 ? dict.stopwatch.start : dict.stopwatch.resume}</button>
            </div>
            {
                laps.length > 0 &&
                <div className="space-y-4 max-h-48 overflow-y-scroll scrollbar-muted">
                    <div className="sticky top-0 bg-foreground space-y-4">
                        <div className="text-sm grid grid-cols-3 text-center gap-4 text-muted-foreground">
                            <span>ラップ</span><span>ラップタイム</span><span>スプリット</span>
                        </div>
