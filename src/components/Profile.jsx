                ...
                <section className="mt-8 px-4 py-4 card border-none flex items-center flex-grow">
                    {
                        isOwner ?
                            (<>
                                🗨️ <input onChange={(e) => { setBio(e.target.value); }} value={bio} type="text" name="bio" id="bio" placeholder="I want to tell everyone that..." className="bg-transparent w-5/6 ml-1 outline-none truncate" />{profile.bio !== bio && <img onClick={handleSaveBio} src={`${fluentRepo}/Floppy%20disk/Color/floppy_disk_color.svg`} className="h-6 w-auto ml-auto cursor-pointer animate-pulse" alt="Fluent floppy disk emoji for saving bio" />}
                            </>)
                            :
                            <p>🗨️ {bio}</p>
                    }
                ...
                <section className="space-y-2 mt-8 rounded-lg flex-1">
                    ...
                    <div className="card space-y-4 px-10 py-8">
                        {profile.study_sessions ? [...profile.study_sessions].sort((a, b) => b.sessions - a.sessions).slice(0, 3).map((session, index) => {
                            return (
                                <div key={index} className="flex justify-between items-center">
                                    <div className="flex gap-2 items-center">
                                        ...
                                        <p className="">{formatDateEn(new Date(session.last_edited))}</p>
                                    </div>
                            ...
                            :
                            <span className="text-muted flex gap-2 text-sm"><img className="w-5 h-auto" src={`${fluentRepo}/Sleeping%20face/Color/sleeping_face_color.svg`} alt="Fluent sleeping face emoji signaling empty session" />Empty session</span>
                        }
                    </div>
                </section>
                <section className="mt-8 space-y-2 flex-1">
                    ...
                    <div className="flex gap-1 justify-center md:justify-around px-4 md:px-16 py-8 card">
                        {sortedWeekDay.map((day, index) => {
                            ...
                            return (
                                <div key={index} className="flex flex-col gap-1 items-center justify-between min-w-9">
